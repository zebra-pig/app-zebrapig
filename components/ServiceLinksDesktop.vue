...
<template>
    <div class="spacer center-layout-height">
        <div class="service-links-desktop" v-if="serviceLinks">
            <div class="container">
                <div class="backdrop">
                    <span class="material-symbols-outlined icon">chevron_right</span>
                </div>
                <nuxt-link class="card" v-for="(link, index) of serviceLinks" :key="link.id"
                    :class="{ active: index === activeService }" @mousemove="emit('setActive', index)"
                    :to="useFullLink(link)">
