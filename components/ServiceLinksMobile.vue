...
<template>
    <div class="spacer center-layout-height">
        <div v-if="serviceLinks" class="links" ref="links">
            <nuxt-link :to="localePath(link.route)" class="title" v-for="(link, index) in serviceLinks" :style="{
                '--pos': index - activeService,
            }">
                {{ link.translations.filter((a: any) => a.language_code.code == locale)[0].title }}
            </nuxt-link>
            <h1 class="arrow-left" @click="goLast()"><span class="material-symbols-outlined">chevron_left</span></h1>
            <h1 class="arrow-right" @click="goNext()"><span class="material-symbols-outlined">chevron_right</span></h1>
        </div>
    </div>
</template>
